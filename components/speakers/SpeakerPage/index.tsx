...
import {
    ...
    IRootState,
    ISpeaker,
    SpeakerPriceCurrencyList, SpeakerPricePrepareTypeList,
...
const SpeakerPage = (props: Props) => {
    ...
    const renderInfoContactItem = (label: string, phone: string, email: string) => {
        ...
        return <div className={styles.infoItem}>
            ...
            <div className={styles.value}>
                ...
                {email &&
                    <a className={cx(styles.contact, {[styles.space]: phone && email})}
    ...
    const renderPriceDescription = (price: IPriceDescription, typeList: { label: string, value: string }[]) => {
        const priceType = typeList?.find(i => i.value === price.name)?.label || price.name;
        const currency = SpeakerPriceCurrencyList?.find(i => i.value === speaker.currency)?.label || '';

        return `${price.price}${currency ? ` ${currency}` : ''}${priceType ? `, ${priceType}` : ''}`
    }
    const renderPriceDescriptionList = (prices: IPriceDescription[], typeList: { label: string, value: string }[]) => {
        return <div className={styles.priceDescriptionList}>{prices.map(i => <div>
            ...
            {(!currentLoading && speaker) && <div className={styles.root}>
                ...
                <div className={styles.content}>
                    ...
                    <div className={styles.tags}>
                        ...
                        />)}
                        {canEdit &&
                            <div className={styles.actions}><ButtonSelect onChange={handleSettingsClick}
                                                                          options={settings}
                                                                          size="9px 20px">Настройки</ButtonSelect>
                            </div>}
                ...
                {speaker &&
                    <SpeakerFeedbackList user={props.user} speakerId={speaker?.id} onEditClick={handleEditFeedbackClick}
